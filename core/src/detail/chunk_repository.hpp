...
//
// Connection-stateless: every method takes the `sqlite3*` to run on, so
// ChunkStore can route writes to the writer connection and reads to a
// pooled read-only connection (ADR-11). The constructor's `db` is used only
// to create the schema. Non-owning throughout -- RetrievalEngine::Impl /
// the pool own every connection and outlive this object.
namespace retrieval_engine::detail {

class ChunkRepository {
public:
    // `db` is used here only to CREATE the documents/chunks/chunks_fts
    // schema; it is not retained.
    ...
    // transaction on `db` (rolled back atomically if anything fails partway
    ...
    std::size_t chunk_count(sqlite3* db) const;
    ...
    ChunkSearchResult Resolve(sqlite3* db, std::uint64_t chunk_id) const;
    ...
    std::vector<ChunkSearchResult> SearchSparse(sqlite3* db, const std::string& query_text, std::size_t k) const;
    ...
    std::int64_t GetCreatedAt(sqlite3* db, const std::string& document_id, std::size_t chunk_index) const;
...
private:
    ...
    void InsertDocumentRow(sqlite3* db, sqlite3_stmt* statement, const DocumentInput& document) const;
    std::uint64_t InsertChunkRow(sqlite3* db, sqlite3_stmt* statement, const std::string& document_id,
                                 std::size_t chunk_index, const DocumentChunkInput& chunk) const;
    void InsertChunkFtsRow(sqlite3* db, sqlite3_stmt* statement, std::uint64_t chunk_id, const std::string& text) const;
