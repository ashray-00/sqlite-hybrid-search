...
def test_reopening_engine_loads_sidecar(tmp_path):
    ...
    engine = retrieval_engine.Engine(db_path, dim=_DIM)
    engine.add(
    ...
    reopened = retrieval_engine.Engine(db_path, dim=_DIM)
...
def test_cli_ingest_creates_sidecar_and_query_reuses_it(tmp_path):
    ...
    assert (tmp_path / ".retrieval_engine.sqlite3.usearch").exists()
